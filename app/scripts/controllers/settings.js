...
angular.module('ngTorrentUiApp')
    .controller('SettingsCtrl', function($scope, torrentServerService, $log, $translate, toastr, $timeout, $cookies, ntuConst) {
        ...
        var getSettingTexts = function(setArr, sectionId, subsectionId) {
            ...
            $translate(setArr[1]).then(function(val) {
                setting.label = val;
            });
            $translate(sectionId).then(function(val) {
                setting.sectionLabel = val;
            });
            $translate(subsectionId).then(function(val) {
                setting.subsectionLabel = val;
            });
        ...
        var initData = function(conf) {
            ...
            var setLabel = function(label, obj) {
                $translate(label).then(function(val) {
                    obj.label = val;
                });
            };
        ...
        $scope.saveSetting = function(setting) {
            ...
            if (service) {
                ...
                ts.success(function() {
                    ...
                    toastr.error('Error saving setting ' + (setting.label !== '') ? setting.label : setting.id, null, {
                        timeOut: 5000
                    });
                });
                return ts;
            } else {
                toastr.warning('Saving settings not supported', null, {
                    timeOut: 1000
                });
            }
        };
